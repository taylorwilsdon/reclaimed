...
def test_icloud_detection(scanner, temp_directory):
...
def test_access_issues(scanner, temp_directory):
...
def test_save_results(scanner, temp_directory):
...
def test_keyboard_interrupt_handling(scanner, temp_directory):
    """Test graceful handling of keyboard interrupts."""
    # Mock the _walk_directory method to raise KeyboardInterrupt
    def mock_walk(*args):
        raise KeyboardInterrupt()
    
    scanner._walk_directory = mock_walk
...
def test_max_results_limit(scanner, temp_directory):
    """Test respecting max_files and max_dirs limits."""
    max_files = 2
    max_dirs = 1
    
    files, dirs = scanner.scan_directory(
        temp_directory, 
        max_files=max_files, 
        max_dirs=max_dirs
    )
